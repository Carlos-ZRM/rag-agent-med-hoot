...
@dsl.component(
    base_image=base_image,
    packages_to_install=["pandas", "scikit-learn", "plotly", "jinja2", "requests"],
)
def evaluate_models(
    pca_dataset: dsl.Input[dsl.Dataset],
    num_class: int,
    best_model_artifact: dsl.Output[dsl.Model],
    evaluation_report: dsl.Output[dsl.HTML],
    ...
    kmeans = KMeans(n_clusters=num_class, n_init=100, init="k-means++", random_state=0)
    kmeans_labels = kmeans.fit_predict(df_pca)

    gmm = GaussianMixture(n_components=num_class, n_init=100, random_state=0)
    gmm_labels = gmm.fit_predict(df_pca)

    # --- Calcular métricas ---
    results = {}
    for name, labels, model in [("kmeans", kmeans_labels, kmeans), ("gmm", gmm_labels, gmm)]:
        sil = silhouette_score(df_pca, labels)
        cal = calinski_harabasz_score(df_pca, labels)
    ...
    for name, metrics in results.items():
        eval_metrics.log_metric(f"{name}_silhouette", metrics["silhouette"])
        eval_metrics.log_metric(f"{name}_calinski_harabasz", metrics["calinski_harabasz"])
        eval_metrics.log_metric(f"{name}_davies_bouldin", metrics["davies_bouldin"])

    # --- Elegir mejor modelo (silhouette más alto) ---
    best_name = max(results, key=lambda k: results[k]["silhouette"])
    best_model = results[best_name]["model"]

    eval_metrics.log_metric("best_model", best_name)
    eval_metrics.log_metric("best_silhouette", results[best_name]["silhouette"])

    # --- Exportar mejor modelo ---
    with open(best_model_artifact.path, "wb") as f:
        pickle.dump(best_model, f)

    best_model_artifact.metadata["model_type"] = best_name
    best_model_artifact.metadata["n_clusters"] = num_class
    best_model_artifact.metadata["silhouette"] = results[best_name]["silhouette"]

    # --- Tabla comparativa HTML ---
    df_comparison = pd.DataFrame({
        "Modelo": ["KMeans", "GMM"],
        "Silhouette": [results["kmeans"]["silhouette"], results["gmm"]["silhouette"]],
        "Calinski-Harabasz": [results["kmeans"]["calinski_harabasz"], results["gmm"]["calinski_harabasz"]],
        "Davies-Bouldin": [results["kmeans"]["davies_bouldin"], results["gmm"]["davies_bouldin"]],
        "Ganador": ["<-- BEST" if best_name == "kmeans" else "", "<-- BEST" if best_name == "gmm" else ""],
    })
    ...
    html = f"""<html><body>
    <h2>Evaluación de Modelos - {num_class} Clusters</h2>
    <h3>Mejor modelo: {best_name.upper()}</h3>
    {styled_html}
    </body></html>"""
    ...
    print(f"Mejor modelo: {best_name} con silhouette={results[best_name]['silhouette']}")
...
def fifa_pipeline(
    ...
    evaluate_task = evaluate_models(
        pca_dataset=feature_engineering_task.outputs["pca_dataset"],
        num_class=num_class,
    ).after(clustering_knn_task, clustering_gmm_task)
